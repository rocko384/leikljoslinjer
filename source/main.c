...
int main(int argc, char** argv) {
	...
	translate_sphere(&sample_sphere1, (vec3) { 3, 0, -3 });
	...
	cylinder cyl1 = UNIT_CYLINDER;

	rotate_cylinder(&cyl1, rotate_on_axis((vec3) { 1, 1, 1 }, 10));
	translate_cylinder(&cyl1, (vec3) { 0, 0, 2 });
	...
	sample_scene.nodes[6] = CREATE_NODE(cylinder, cyl1, cyl_mat, ray_intersect_cylinder);

	sample_scene.lights[0] = create_point_light((color) { 1, 1, 1 }, (vec3) { 0, 10, 0 }, 18);
	sample_scene.lights[1] = create_point_light((color) { 1, 1, 1 }, (vec3) { 7, 0, -7 }, 20);
